...
export const profile = {
  ...
  competencies: [
    'MMO 서버를 백지에서 반복 설계·구축 — 네트워크·동시성(락프리)·네비게이션·서버 물리·가상 액터 기반 분산 서버',
...
export const sections = [
  ...
  // ───────── 현재 · 마무리 ─────────
  {
    ...
    accent: 'blue',
    num: '16',
    kicker: 'NCSOFT · Panthera · AI 개발 생산성 향상 주도',
    title: 'AI 개발 생산성 향상',
...
export const projects = [
  {
    ...
    private: true,
    tech: 'Godot · C#',
    metric: '동시 1억 2,800만 마리',
    points: [
      '노드/GC 없는 데이터지향 엔진 — 모든 엔티티를 SoA 플랫 배열로, 시스템당 드로우콜 1개(MultiMesh)·물리엔진 대신 배열 기반 공간해시로 동시 1억 2,800만 마리 시뮬레이션 + 만렙 스킬 전투 실측 (스폰·렌더까지는 1억 9,200만 확인)',
      ...
      '셰이더 인스턴싱 — 아틀라스 프레임·피격 플래시를 INSTANCE_CUSTOM 인스턴스 데이터로 셰이더에서 처리',
      '이동·분리 병렬화 — 마리당 상태는 자기 인덱스에만 쓰는 구조라 Parallel.ForEach가 안전. 사격·자폭 부수효과는 Interlocked 큐에 모아 메인 스레드에서 드레인, 난수는 결정론적 해시로 대체 → 코어 수에 비례해 확장',
      '그리드 빌드 융합 — 직렬 100만 삽입 루프 대신 병렬 이동 패스 안에서 무락(lock-free) 버킷 푸시로 재구축, 마지막 직렬 O(n) 단계를 제거해 다중코어 암달 병목 방지',
      '천장의 정체를 규명 — 연산이 아니라 메모리(SoA 마리당 112바이트, 64M=7.9GB·128M=15.3GB 선형 일치). 자료형 다이어트로 54B까지 줄이면 2.7억이 가능하다는 예산표까지 산출',
    ],
    media: [
      { src: 'hellfarm/battle_100m.mp4', poster: 'hellfarm/battle_100m.jpg', cap: '1억 마리 안에서 만렙 스킬 6종이 구멍을 뚫는 중 — 5초에 208만 마리 처치. HUD의 obj 99,549,966이 실측값 (i5-9600K 6코어 · 봇 셀프플레이 인게임 녹화)' },
    ],
  },
  {
    name: 'ZeroLlama',
    ...
    points: [
      '자동 튜닝 — 복잡한 옵션을 몰라도 GPU·VRAM에 맞춰 적합한 모델·양자화를 제안하고 KV 캐시 크기·GPU에 올릴 레이어 수(ngl)·컨텍스트 크기를 최적으로 설정. 근거도 함께 보여주고, MoE 전문가를 CPU RAM으로 오프로드해 8GB VRAM으로도 35B급(활성 ~3B) 모델을 구동',
      '양자화 모델용 도구 재설계 — 작은 모델·작은 컨텍스트에서도 쓸 수 있게 시스템 프롬프트를 통상의 1/8로 줄이고, 편집 블록을 9가지 방식으로 대조해 공백·들여쓰기가 어긋나도 적용, <think>는 다음 턴 전 제거, 수정 후 린터로 에러를 되돌림',
      '도구 호출에 의존하지 않는 메모리 — 작은 모델이 메모리 도구를 자주 빠뜨리는 문제를 피해 저장·회수를 모델 밖에서 처리, 노이즈를 걸러 불필요한 회수는 줄이면서 필요한 기억은 유지',
      '작은 컨텍스트에 모든 입력을 담는 설계 — 로그는 핵심 줄만 요약(수천 줄→수십 줄), 저장소는 중요도로 추려 넣고, 창이 차오르면 마무리 유도·대화 압축으로 추론 여유 확보',
